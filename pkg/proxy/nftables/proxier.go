...
type Proxier struct {
	...
	clusterIPs          *nftContainer[[1]string, [0]string]
	serviceIPs          *nftContainer[[3]string, [1]string]
	firewallIPs         *nftContainer[[3]string, [1]string]
	noEndpointServices  *nftContainer[[3]string, [1]string]
	noEndpointNodePorts *nftContainer[[2]string, [1]string]
	serviceNodePorts    *nftContainer[[2]string, [1]string]
}
...
func ensureChain(chain string, tx *knftables.Transaction, createdChains sets.Set[string], existingChainRules map[string]map[string]*int) {
	...
	if existingChainRules != nil {
		if _, ok := existingChainRules[chain]; ok {
			return
		}
	}
...
func (proxier *Proxier) setupNFTables(tx *knftables.Transaction) {
	...
	for _, c := range nftablesJumpChains {
		ensureChain(c.dstChain, tx, createdChains, nil)
	...
	for _, chain := range []string{servicesChain, clusterIPsCheckChain, masqueradingChain, markMasqChain} {
		ensureChain(chain, tx, createdChains, nil)
	}
	...
	ensureChain(firewallCheckChain, tx, createdChains, nil)
	tx.Add(&knftables.Rule{
	...
	// Read the latest containers state from the nftables
	proxier.clusterIPs = newNftContainer[[1]string, [0]string](proxier, "set", clusterIPsSet, tx)
	proxier.serviceIPs = newNftContainer[[3]string, [1]string](proxier, "map", serviceIPsMap, tx)
	proxier.firewallIPs = newNftContainer[[3]string, [1]string](proxier, "map", firewallIPsMap, tx)
	proxier.noEndpointServices = newNftContainer[[3]string, [1]string](proxier, "map", noEndpointServicesMap, tx)
	proxier.noEndpointNodePorts = newNftContainer[[2]string, [1]string](proxier, "map", noEndpointNodePortsMap, tx)
	proxier.serviceNodePorts = newNftContainer[[2]string, [1]string](proxier, "map", serviceNodePortsMap, tx)
}
...
func (c *nftContainer[T1, T2]) resetLeftoverKeys() {
	c.leftoverKeys = sets.New[T1]()
	for key := range c.elements {
		c.leftoverKeys.Insert(key)
	}
}
...
func (c *nftContainer[T1, T2]) ensureElem(elem *knftables.Element, tx *knftables.Transaction) {
	...
	if ok {
		if existingValue != newValue {
			...
			tx.Add(elem)
			c.elements[newKey] = newValue
		}
		delete(c.leftoverKeys, newKey)
	} else {
		tx.Add(elem)
		c.elements[newKey] = newValue
	}
...
func (c *nftContainer[T1, T2]) cleanupLeftoverKeys(getKeySlice func(T1) []string, tx *knftables.Transaction) {
	for key := range c.leftoverKeys {
		e := &knftables.Element{
			Key: getKeySlice(key),
		}
...
func (proxier *Proxier) syncProxyRules() {
	...
	// Reset leftover keys for this iteration
	proxier.clusterIPs.resetLeftoverKeys()
	proxier.serviceIPs.resetLeftoverKeys()
	proxier.firewallIPs.resetLeftoverKeys()
	proxier.noEndpointServices.resetLeftoverKeys()
	proxier.noEndpointNodePorts.resetLeftoverKeys()
	proxier.serviceNodePorts.resetLeftoverKeys()

	listStart := time.Now()
	rules, err := proxier.nftables.ListRules(context.TODO(), "")
	if err != nil {
		proxier.logger.Error(err, "Failed to list nftables rules")
		//nothing to do, chains are flushed by ensureChain
	}
	proxier.logger.V(2).Info("List rules", "elapsed", time.Since(listStart), "len", len(rules))
	// map[chainName][rule1, ..., ruleN]
	existingChainRules := map[string]map[string]*int{}
	for _, rule := range rules {
		if _, ok := existingChainRules[rule.Chain]; !ok {
			existingChainRules[rule.Chain] = map[string]*int{}
		}
		existingChainRules[rule.Chain][rule.Rule] = rule.Handle
	}

	listStart = time.Now()
	chains, err := proxier.nftables.List(context.TODO(), "chains")
	if err != nil {
		proxier.logger.Error(err, "Failed to list nftables chains")
		//nothing to do, chains are flushed by ensureChain
	}
	proxier.logger.V(2).Info("List chains", "elapsed", time.Since(listStart))
	for _, chain := range chains {
		if _, ok := existingChainRules[chain]; !ok {
			existingChainRules[chain] = map[string]*int{}
		}
	}
	...
	for svcName, svc := range proxier.svcPortMap {
		...
		// svcTx is used for all ops except ones that are based on read data from nftables.
		// For now that is everything related to clusterIPsSet...serviceNodePortsMap
		// Since these maps/sets are flushed on read failure, changes either should be also transacted or they won't be
		// needed based on map/set data.
		svcTx := tx
		...
		for _, ep := range allLocallyReachableEndpoints {
			if epInfo, ok := ep.(*endpointInfo); ok {
				ensureChain(epInfo.chainName, svcTx, activeChains, existingChainRules)
			}
		}
		...
		if usesClusterPolicyChain {
			ensureChain(clusterPolicyChain, svcTx, activeChains, existingChainRules)
		}
		...
		if usesLocalPolicyChain {
			ensureChain(localPolicyChain, svcTx, activeChains, existingChainRules)
		}
		...
		if usesExternalTrafficChain {
			ensureChain(externalTrafficChain, svcTx, activeChains, existingChainRules)
		}
		...
		proxier.clusterIPs.ensureElem(&knftables.Element{
			Set: clusterIPsSet,
			Key: []string{svcInfo.ClusterIP().String()},
		}, tx)
		if hasInternalEndpoints {
			proxier.serviceIPs.ensureElem(&knftables.Element{
				Map: serviceIPsMap,
				Key: []string{
			...
			// No endpoints.
			proxier.noEndpointServices.ensureElem(&knftables.Element{
		...
		if usesFWChain {
			ensureChain(fwChain, svcTx, activeChains, existingChainRules)
			var sources []string
			allowFromNode := false
			...
			ensureRule(&knftables.Rule{
				Chain: fwChain,
				Rule: knftables.Concat(
					ipX, "saddr", "!=", "{", sources, "}",
					"drop",
				),
			}, existingChainRules, tx)
		}
		...
		for _, lbip := range svcInfo.LoadBalancerVIPs() {
			...
			if usesFWChain {
				proxier.firewallIPs.ensureElem(&knftables.Element{
					Map: firewallIPsMap,
					Key: []string{
						lbip.String(),
						protocol,
		...
		if svcInfo.NodePort() != 0 {
			if hasEndpoints {
				...
				proxier.serviceNodePorts.ensureElem(&knftables.Element{
			...
			if !hasExternalEndpoints {
				...
				// external traffic (DROP anything that didn't get
				// short-circuited by the EXT chain.)
				proxier.noEndpointNodePorts.ensureElem(&knftables.Element{
		...
		if usesClusterPolicyChain {
			proxier.writeServiceToEndpointRules(svcTx, svcInfo, clusterPolicyChain, clusterEndpoints, existingChainRules)
		}

		// If Local policy is in use, create rules jumping from localPolicyChain
		// to the localEndpoints
		if usesLocalPolicyChain {
			proxier.writeServiceToEndpointRules(svcTx, svcInfo, localPolicyChain, localEndpoints, existingChainRules)
		}
	...
	deletedChains := 0
	deletedRules := 0
	for chain, chainRules := range existingChainRules {
		if isServiceChainName(chain) {
			if !activeChains.Has(chain) {
				...
				delete(proxier.staleChains, chain)
				for rule, handle := range chainRules {
					deletedRules += 1
	...
	proxier.logger.V(2).Info("Deleted stale nftables chains", "activeChains", activeChains.Len(), "numChains", deletedChains, "numRules", deletedRules)

	// OTOH, we can immediately delete any stale affinity sets
	listStart = time.Now()
	existingSets, err := proxier.nftables.List(context.TODO(), "sets")
	proxier.logger.V(2).Info("List sets", "elapsed", time.Since(listStart))
	if err == nil {
	...
	proxier.clusterIPs.cleanupLeftoverKeys(func(key [1]string) []string { return key[:] }, tx)
	proxier.serviceIPs.cleanupLeftoverKeys(func(key [3]string) []string { return key[:] }, tx)
	proxier.firewallIPs.cleanupLeftoverKeys(func(key [3]string) []string { return key[:] }, tx)
	proxier.noEndpointServices.cleanupLeftoverKeys(func(key [3]string) []string { return key[:] }, tx)
	proxier.noEndpointNodePorts.cleanupLeftoverKeys(func(key [2]string) []string { return key[:] }, tx)
	proxier.serviceNodePorts.cleanupLeftoverKeys(func(key [2]string) []string { return key[:] }, tx)
	...
	proxier.logger.V(2).Info("nftables TX succeeded", "elapsed", time.Since(txStart), "tx.len", tx.Len())
	success = true
...
func ensureObj(obj knftables.Object, tx *knftables.Transaction) {
	if tx != nil {
		tx.Add(obj)
	}
}

func ensureRule(rule *knftables.Rule, existingChainRules map[string]map[string]*int, tx *knftables.Transaction) {
	chainRules, ok := existingChainRules[rule.Chain]
	if ok {
		if _, ok2 := chainRules[rule.Rule]; ok2 {
			delete(chainRules, rule.Rule)
			return
		}
	}
	tx.Add(rule)
...
func (proxier *Proxier) writeServiceToEndpointRules(tx *knftables.Transaction, svcInfo *servicePortInfo, svcChain string,
	endpoints []proxy.Endpoint, existingChainRules map[string]map[string]*int) {
	...
	// TODO figure out if endpoints are already sorted
	sort.Slice(endpoints, func(i, j int) bool {
		return endpoints[i].IP() < endpoints[j].IP()
	})

	// Now write loadbalancing rule
	elements := make([]string, 0, len(endpoints)*4)
	for i, ep := range endpoints {
		...
		chainName := epInfo.chainName
		if i != len(endpoints)-1 {
			chainName += ","
		}

		elements = append(elements,
			strconv.Itoa(i), ":", "goto", chainName,
		)
